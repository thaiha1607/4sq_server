...
func changeOrderStatusToShipped(app *pocketbase.PocketBase) {
	app.OnRecordAfterUpdateRequest("internal_orders").Add(func(e *core.RecordUpdateEvent) error {
		internalOrders, err := dbquery.GetInternalOrdersByOrderId(app.Dao(), e.Record.GetString("rootOrderId"))
		if err != nil {
			app.Logger().Error("Failed to get internal orders", "error", err)
			return nil
		}
		order, err := dbquery.GetSingleOrder(app.Dao(), e.Record.GetString("rootOrderId"))
		if err != nil {
			app.Logger().Error("Failed to get order", "error", err)
			return nil
		}
		allShipped := len(lo.Filter(internalOrders, func(internalOrder *custom_models.InternalOrder, _ int) bool {
			return internalOrder.StatusCodeId == order_status.Shipped.ID()
		})) == len(internalOrders)
		isThisInternalOrderShipped := e.Record.GetString("statusCodeId") == order_status.Shipped.ID()
		if (order.StatusCodeId == order_status.Processing.ID() ||
			order.StatusCodeId == order_status.WaitingForAction.ID() ||
			order.StatusCodeId == order_status.PartiallyShipped.ID()) &&
			(allShipped || isThisInternalOrderShipped) {
			var newStatusCodeId string
			if allShipped {
				newStatusCodeId = order_status.Shipped.ID()
			} else {
				newStatusCodeId = order_status.PartiallyShipped.ID()
			}
...
func changeOrderStatusToProcessing(app *pocketbase.PocketBase) {
	app.OnRecordAfterCreateRequest("internal_orders").Add(func(e *core.RecordCreateEvent) error {
		order, err := dbquery.GetSingleOrder(app.Dao(), e.Record.GetString("rootOrderId"))
		if err != nil {
			app.Logger().Error("Failed to get order", "error", err)
			return nil
		}
		if order.StatusCodeId == order_status.Pending.ID() {
			...
			app.Logger().Info("Order status changed to Processing", "order_id", order.Id)
		}
		return nil
	})
}
